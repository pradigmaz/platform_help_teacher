...
export function NoteButton({ entityType, entityId, size = 'sm', className }: NoteButtonProps) {
  ...
  const handleOpenChange = async (nextOpen: boolean) => {
    setIsOpen(nextOpen);
    if (!nextOpen || !notesContext || notesContext.hasLoaded(entityType, entityId)) {
      return;
    }
    await notesContext.loadNotesBatch(entityType, [entityId]);
  };

  return (
    <Popover open={isOpen} onOpenChange={(nextOpen) => void handleOpenChange(nextOpen)}>
      <PopoverTrigger
        type="button"
        className={cn(
