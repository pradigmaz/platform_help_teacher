...
async def check_admin_links():
    async with AsyncSessionLocal() as db:
        ...
        result = await db.execute(
            select(TeacherSubjectAssignment).where(TeacherSubjectAssignment.teacher_id == admin.id)
        )
        ...
        result = await db.execute(select(ScheduleItem).where(ScheduleItem.teacher_id == admin.id))
        ...
        result = await db.execute(select(func.count()).select_from(ScheduleItem).where(ScheduleItem.teacher_id == None))
        orphan_count = result.scalar()
        print(f"   • Элементов без teacher_id: {orphan_count}")

        if orphan_count > 0:
            result = await db.execute(select(ScheduleItem).where(ScheduleItem.teacher_id == None).limit(5))
            orphans = result.scalars().all()
            for item in orphans:
                print(
                    f"      • {item.day_of_week.value} #{item.lesson_number}: {item.subject} (group_id={item.group_id})"
                )
