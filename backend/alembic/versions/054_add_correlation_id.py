...
def upgrade() -> None:
    ...
    op.add_column("student_audit_log", sa.Column("correlation_id", sa.String(64), nullable=True))

    # Индекс для поиска по correlation_id
    op.create_index("idx_audit_correlation_id", "student_audit_log", ["correlation_id"])
