...
def upgrade() -> None:
    ...
    op.add_column(
        "users", sa.Column("contacts", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default="{}")
    )

    # Add contact_visibility JSONB column with default empty object
    op.add_column(
        "users",
        sa.Column("contact_visibility", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default="{}"),
    )
