...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            postgresql.ENUM("PRESENT", "ABSENT", "LATE", "EXCUSED", name="attendance_status_enum", create_type=False),
    ...
    op.create_check_constraint(
        "check_file_required_if_not_manual", "submissions", "(is_manual IS TRUE) OR (s3_key IS NOT NULL)"
    )
