...
def upgrade() -> None:
    ...
    admin_result = conn.execute(sa.text("SELECT id FROM users WHERE role = 'ADMIN' LIMIT 1"))
    ...
    subjects_result = conn.execute(sa.text("SELECT id, name FROM subjects WHERE is_active = true"))
    ...
    total_assignments = conn.execute(
        sa.text("SELECT COUNT(*) FROM teacher_subject_assignments WHERE teacher_id = :admin_id"), {"admin_id": admin_id}
    ).scalar()

    total_schedule = conn.execute(
        sa.text("SELECT COUNT(*) FROM schedule_items WHERE teacher_id = :admin_id"), {"admin_id": admin_id}
    ).scalar()
