...
def upgrade() -> None:
    ...
    op.create_index("idx_labs_deleted_at", "labs", ["deleted_at"], postgresql_where=sa.text("deleted_at IS NULL"))

    # Add deleted_at to submissions
    op.add_column("submissions", sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True))
    op.create_index(
        "idx_submissions_deleted_at", "submissions", ["deleted_at"], postgresql_where=sa.text("deleted_at IS NULL")
    )
