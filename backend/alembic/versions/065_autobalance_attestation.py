...
def upgrade() -> None:
    ...
    # === ПЕРЕИМЕНОВАНИЕ ===
    op.alter_column("attestation_settings", "activity_weight", new_column_name="activity_reserve")

    # === ДОБАВЛЕНИЕ КОЭФФИЦИЕНТОВ ===
    # Коэффициенты оценок (grade_5=1.0 и grade_2=0.0 фиксированы, не хранятся)
    op.add_column("attestation_settings", sa.Column("grade_4_coef", sa.Float(), nullable=False, server_default="0.7"))
    op.add_column("attestation_settings", sa.Column("grade_3_coef", sa.Float(), nullable=False, server_default="0.4"))

    # Количество работ
    op.add_column(
        "attestation_settings", sa.Column("labs_count_first", sa.Integer(), nullable=False, server_default="8")
    )
    op.add_column(
        "attestation_settings", sa.Column("labs_count_second", sa.Integer(), nullable=False, server_default="10")
    )
    op.add_column(
        "attestation_settings", sa.Column("self_works_count", sa.Integer(), nullable=False, server_default="2")
    )
    op.add_column(
        "attestation_settings", sa.Column("colloquium_count", sa.Integer(), nullable=False, server_default="1")
    )

    # Коэффициент опоздания
    op.add_column("attestation_settings", sa.Column("late_coef", sa.Float(), nullable=False, server_default="0.5"))
...
def downgrade() -> None:
    ...
    op.alter_column("attestation_settings", "activity_reserve", new_column_name="activity_weight")

    # === ВОССТАНОВЛЕНИЕ УДАЛЁННЫХ ПОЛЕЙ ===
    op.add_column(
        "attestation_settings", sa.Column("participation_points", sa.Float(), nullable=False, server_default="0.5")
    )

    op.add_column("attestation_settings", sa.Column("absent_points", sa.Float(), nullable=False, server_default="-0.1"))
    op.add_column("attestation_settings", sa.Column("late_points", sa.Float(), nullable=False, server_default="0.5"))
    op.add_column("attestation_settings", sa.Column("present_points", sa.Float(), nullable=False, server_default="1.0"))
    ...
    op.add_column(
        "attestation_settings", sa.Column("required_labs_count", sa.Integer(), nullable=False, server_default="5")
    )
