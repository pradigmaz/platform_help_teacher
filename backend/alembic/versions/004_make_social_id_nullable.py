...
def upgrade() -> None:
    ...
    op.alter_column("users", "social_id", existing_type=sa.BIGINT(), nullable=True)
...
def downgrade() -> None:
    ...
    op.execute(
        "DELETE FROM users WHERE social_id IS NULL"
    )  # Очистка перед откатом (опасно, но необходимо для целостности)
    op.alter_column("users", "social_id", existing_type=sa.BIGINT(), nullable=False)
