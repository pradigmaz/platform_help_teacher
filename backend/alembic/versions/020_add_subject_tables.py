...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column(
            "teacher_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False
        ),
        sa.Column(
            "subject_id",
        ...
        sa.Column(
            "group_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("groups.id", ondelete="CASCADE"), nullable=True
        ),
    ...
    op.create_foreign_key(
        "fk_schedule_items_subject", "schedule_items", "subjects", ["subject_id"], ["id"], ondelete="SET NULL"
    )
    ...
    op.create_foreign_key("fk_works_subject", "works", "subjects", ["subject_id"], ["id"], ondelete="SET NULL")

    # 5. Add group_id to works (optional)
    op.add_column("works", sa.Column("group_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.create_foreign_key("fk_works_group", "works", "groups", ["group_id"], ["id"], ondelete="SET NULL")
