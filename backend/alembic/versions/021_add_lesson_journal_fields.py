...
def upgrade() -> None:
    ...
    op.create_foreign_key("fk_lessons_subject", "lessons", "subjects", ["subject_id"], ["id"], ondelete="SET NULL")
    ...
    op.create_table(
        ...
        sa.Column(
            "lesson_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("lessons.id", ondelete="CASCADE"), nullable=False
        ),
        sa.Column(
            "student_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id", ondelete="CASCADE"), nullable=False
        ),
    ...
    op.create_unique_constraint(
        "uq_lesson_grade_student_lesson_work", "lesson_grades", ["lesson_id", "student_id", "work_number"]
    )
