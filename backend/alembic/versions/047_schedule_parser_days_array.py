...
def upgrade() -> None:
    ...
    op.add_column("schedule_parser_configs", sa.Column("days_of_week", ARRAY(sa.Integer), nullable=True))

    # 2. Мигрируем данные: day_of_week -> days_of_week (как массив из одного элемента)
    op.execute("UPDATE schedule_parser_configs SET days_of_week = ARRAY[day_of_week] WHERE day_of_week IS NOT NULL")
...
def downgrade() -> None:
    ...
    op.add_column("schedule_parser_configs", sa.Column("day_of_week", sa.Integer, nullable=True))

    # 2. Мигрируем данные: берём первый элемент массива
    op.execute("UPDATE schedule_parser_configs SET day_of_week = days_of_week[1] WHERE days_of_week IS NOT NULL")

    # 3. Устанавливаем default
    op.alter_column("schedule_parser_configs", "day_of_week", server_default="6", nullable=False)
