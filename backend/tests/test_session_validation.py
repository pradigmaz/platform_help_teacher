...
3. Без session cookie (backward compatibility) → 200 OK с warning в логах
...
async def test_get_current_user_with_valid_session(
    ...
    with patch('app.api.deps.session_service.validate_session', 
...
@pytest.mark.asyncio
async def test_get_current_user_with_valid_session_logs_debug(
    test_user,
    mock_db_session,
    valid_session_data,
    caplog
...
@pytest.mark.asyncio
async def test_get_current_user_with_revoked_session(
    test_user,
    mock_db_session
):
    """
...
@pytest.mark.asyncio
async def test_get_current_user_with_revoked_session_logs_warning(
    test_user,
    mock_db_session,
    caplog
):
...
# ============================================================================
# Tests: Без session cookie (backward compatibility)
# ============================================================================

@pytest.mark.asyncio
async def test_get_current_user_without_session_cookie(
    test_user,
    mock_db_session
):
    """
    Тест: JWT валиден, но нет session cookie → 200 OK (backward compatibility).
    """
    # Arrange
    token = create_jwt_token(test_user.id)
...
@pytest.mark.asyncio
async def test_get_current_user_without_session_cookie_logs_warning(
    test_user,
    mock_db_session,
    caplog
):
    """
    Тест: Отсутствие session cookie логирует warning.
    """
    # Arrange
    import logging
    caplog.set_level(logging.WARNING)
...
@pytest.mark.asyncio
async def test_get_current_user_with_expired_session_in_redis(
    test_user,
    mock_db_session
):
    """
...
@pytest.mark.asyncio
async def test_get_current_user_with_nonexistent_session_id(
    test_user,
    mock_db_session
):
    """
...
@pytest.mark.asyncio
async def test_get_current_user_with_impersonation_session(
    test_user,
    mock_db_session
):
    """
...
@pytest.mark.asyncio
async def test_get_current_user_inactive_user_with_valid_session(
    test_user,
    mock_db_session,
    valid_session_data
):
...
@pytest.mark.asyncio
async def test_get_current_user_with_malformed_session_data(
    test_user,
    mock_db_session
):
    """
    Тест: Если session_service.validate_session возвращает некорректные данные.
    
    В реальности validate_session возвращает либо dict, либо None,
    но проверяем что код обрабатывает любой truthy результат.
    """
    # Arrange
    token = create_jwt_token(test_user.id)
    ...
    # Return malformed but truthy dict
    with patch('app.api.deps.session_service.validate_session',
               new_callable=AsyncMock) as mock_validate:
        mock_validate.return_value = {"unexpected": "value"}
        
        # Act
        user = await get_current_user(request, mock_db_session)
        
        # Assert - должно работать, т.к. {} это truthy
        assert user.id == test_user.id


@pytest.mark.asyncio
async def test_get_current_user_with_very_long_session_id(
    test_user,
    mock_db_session,
    valid_session_data
):
...
@pytest.mark.asyncio
async def test_get_current_user_multiple_calls_same_session(
    test_user,
    mock_db_session,
    valid_session_data
):
