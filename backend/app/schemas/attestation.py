...
class AttestationSettingsBase(BaseModel):
    ...
    # === ПЕРИОДЫ ===
    period_start_date: date | None = Field(default=None, description="Начало периода")
...
class ComponentBreakdown(BaseModel):
    ...
    # Опциональные
    self_works_score: float | None = Field(default=None)
    colloquium_score: float | None = Field(default=None)
