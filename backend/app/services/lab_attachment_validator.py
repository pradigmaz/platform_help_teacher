...
class AttachmentValidationResult:
    ...
    is_valid: bool
    blocking_lab_number: Optional[int] = None
    can_attach_from: Optional[date] = None
    message: str = ""
...
class LabAttachmentValidator:
    ...
    async def validate_attachment(
        ...
        prev_lab_lessons = await self._get_lessons_with_work_number(
            group_id, subject_id, prev_lab_number
        )
        ...
        unique_labs_count = await self._count_unique_labs_from(
            first_lesson_date, group_id, subject_id
        )
    ...
    async def _count_unique_labs_from(
        self,
        from_date: date,
        group_id: UUID,
        subject_id: Optional[UUID]
        ...
        query = (
            select(func.count(func.distinct(Lesson.work_number)))
            .where(and_(*filters))
        )
        result = await self.db.execute(query)
    ...
    async def _get_attachment_available_date(
        ...
        query = (
            select(Lesson.date, Lesson.work_number)
            .where(and_(*filters))
