...
# ВАЖНО: audio, screen и webgl убраны — слишком много коллизий между пользователями
# В компьютерных классах у всех одинаковые видеокарты → одинаковый webgl → массовые баны
BANNABLE_COMPONENTS = [
    "canvas",      # Canvas fingerprint — уникален для браузера+GPU
    # "webgl",     # ОТКЛЮЧЕНО: в компьютерных классах одинаковые GPU → массовые баны
