...
def get_user(user_id: int):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=em.USER_NOT_FOUND)
...
def validate_file_type(file_type: str, allowed_types: list[str]):
    ...
    if file_type not in allowed_types:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=em.format_error(em.FILE_TYPE_NOT_ALLOWED, type=file_type)
        )
...
def process_file(file_size: int, max_size: int):
    ...
    if file_size > max_size:
        ...
        logger.error(f"[Files:validate] {error_msg}", extra={"file_size": file_size, "max_size": max_size})
...
class GroupService:
    ...
    async def get_group(self, group_id: int):
        ...
        if not group:
            logger.warning(f"[GroupService:get_group] {em.GROUP_NOT_FOUND}", extra={"group_id": group_id})
...
def validate_permissions(user_role: str, required_role: str):
    ...
    if not user_role:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail=em.USER_NOT_FOUND)

    if user_role != required_role:
        logger.warning(
            f"[Auth:validate] {em.NOT_ENOUGH_PERMISSIONS}",
...
def check_lab_availability(lab, current_date):
    ...
    if lab.available_from and current_date < lab.available_from:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=em.LAB_NOT_AVAILABLE_YET)

    if not lab.is_available_by_schedule:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=em.LAB_NOT_AVAILABLE_BY_SCHEDULE)
